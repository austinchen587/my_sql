    ...
    const htmlContent = `
        ...
        <div class="modal-body custom-scrollbar" style="max-height: 75vh; overflow-y: auto;">
            ...
            ${renderSectionAsTable('商品信息', 
                [data.commodity_names, data.parameter_requirements, data.purchase_quantities, data.control_amounts, data.suggested_brands],
            ...
            ${renderSectionAsTable('商务要求', 
                data.business_items && data.business_items.length > 0 ? 
                    [data.business_items, data.business_requirements] : 
                    [Array.from({length: Math.max(data.business_requirements?.length || 0, 1)}, (_, i) => i + 1), data.business_requirements],
                ['商务项目', '商务要求']
            )}
